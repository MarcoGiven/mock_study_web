...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    thisExp.addLoop(colorBlock)  # add the loop to the experiment
    thisColorBlock = colorBlock.trialList[0]  # so we can initialise stimuli with some values
    # abbreviate parameter names if possible (e.g. rgb = thisColorBlock.rgb)
    if thisColorBlock != None:
        for paramName in thisColorBlock:
            globals()[paramName] = thisColorBlock[paramName]
    
    for thisColorBlock in colorBlock:
        colorBlock.status = STARTED
        if hasattr(thisColorBlock, 'status'):
            thisColorBlock.status = STARTED
        currentLoop = colorBlock
        ...
        trialsColor = data.TrialHandler2(
            ...
            extraInfo=expInfo, 
            originPath=-1, 
            trialList=data.importConditions('hotel_color.xlsx'), 
            seed=None, 
        )
        ...
        while continueRoutine:
            # if trial has changed, end Routine now
            if hasattr(thisColorBlock, 'status') and thisColorBlock.status == STOPPING:
                continueRoutine = False
            # get current time
            t = routineTimer.getTime()
            tThisFlip = win.getFutureFlipTime(clock=routineTimer)
        ...
        if hasattr(thisColorBlock, 'status'):
            thisColorBlock.status = FINISHED
        # if awaiting a pause, pause now
        if colorBlock.status == PAUSED:
            thisExp.status = PAUSED
            pauseExperiment(
                thisExp=thisExp, 
                win=win, 
    ...
    thisExp.addLoop(grayBlock)  # add the loop to the experiment
    thisGrayBlock = grayBlock.trialList[0]  # so we can initialise stimuli with some values
    # abbreviate parameter names if possible (e.g. rgb = thisGrayBlock.rgb)
    if thisGrayBlock != None:
        for paramName in thisGrayBlock:
            globals()[paramName] = thisGrayBlock[paramName]
    
    for thisGrayBlock in grayBlock:
        grayBlock.status = STARTED
        if hasattr(thisGrayBlock, 'status'):
            thisGrayBlock.status = STARTED
        currentLoop = grayBlock
        thisExp.timestampOnFlip(win, 'thisRow.t', format=globalClock.format)
        # abbreviate parameter names if possible (e.g. rgb = thisGrayBlock.rgb)
        if thisGrayBlock != None:
            for paramName in thisGrayBlock:
        ...
        while continueRoutine:
            # if trial has changed, end Routine now
            if hasattr(thisGrayBlock, 'status') and thisGrayBlock.status == STOPPING:
                continueRoutine = False
            # get current time
            t = routineTimer.getTime()
            tThisFlip = win.getFutureFlipTime(clock=routineTimer)
        ...
        if hasattr(thisGrayBlock, 'status'):
            thisGrayBlock.status = FINISHED
        # if awaiting a pause, pause now
        if grayBlock.status == PAUSED:
            thisExp.status = PAUSED
            pauseExperiment(
                thisExp=thisExp, 
                win=win, 
    ...
    thisExp.addLoop(bothBlock)  # add the loop to the experiment
    thisBothBlock = bothBlock.trialList[0]  # so we can initialise stimuli with some values
    # abbreviate parameter names if possible (e.g. rgb = thisBothBlock.rgb)
    if thisBothBlock != None:
        for paramName in thisBothBlock:
            globals()[paramName] = thisBothBlock[paramName]
    
    for thisBothBlock in bothBlock:
        bothBlock.status = STARTED
        if hasattr(thisBothBlock, 'status'):
            thisBothBlock.status = STARTED
        currentLoop = bothBlock
        thisExp.timestampOnFlip(win, 'thisRow.t', format=globalClock.format)
        # abbreviate parameter names if possible (e.g. rgb = thisBothBlock.rgb)
        if thisBothBlock != None:
            for paramName in thisBothBlock:
        ...
        while continueRoutine:
            # if trial has changed, end Routine now
            if hasattr(thisBothBlock, 'status') and thisBothBlock.status == STOPPING:
                continueRoutine = False
            # get current time
            t = routineTimer.getTime()
            tThisFlip = win.getFutureFlipTime(clock=routineTimer)
        ...
        if hasattr(thisBothBlock, 'status'):
            thisBothBlock.status = FINISHED
        # if awaiting a pause, pause now
        if bothBlock.status == PAUSED:
            thisExp.status = PAUSED
            pauseExperiment(
                thisExp=thisExp, 
                win=win, 
